...
carregarFilmesPredefinidos(listaFilme);
...
//backend->frontend
function carregarFilmesPredefinidos() {
    const filmesPredefinidos = [
        new Filme(1, "O Poderoso Chefão", "Drama", "1972-03-24", 175, "Paramount Pictures", "Francis Ford Coppola"),
        new Filme(2, "Batman: O Cavaleiro das Trevas", "Ação", "2008-07-18", 152, "Warner Bros", "Christopher Nolan"),
        new Filme(3, "Forrest Gump", "Comédia", "1994-07-06", 142, "Paramount Pictures", "Robert Zemeckis"),
        new Filme(4, "Star Wars: Uma Nova Esperança", "Ficção Científica", "1977-05-25", 121, "Lucasfilm", "George Lucas"),
        new Filme(5, "Matrix", "Ficção Científica", "1999-03-31", 136, "Warner Bros", "Lana Wachowski, Lilly Wachowski")
    ];

    listaFilme = filmesPredefinidos;
    listar(); // Exibe os filmes na tela
}
...
function mostrarDadosFilme(filme) {
    ...
    let generoSelect = document.getElementById("inputGenero");
    let generoValue = filme.genero.toLowerCase(); // transforma em minúsculas para garantir a correspondência

    // Encontra a opção com o valor correspondente e seleciona-a
    for (let i = 0; i < generoSelect.options.length; i++) {
        if (generoSelect.options[i].value === generoValue) {
