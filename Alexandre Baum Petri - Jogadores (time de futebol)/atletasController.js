...
document.getElementById("inputNum").addEventListener("focus", function () {
...
function procurePorChavePrimaria(chave) {
    for (let i = 0; i < listaAtletas.length; i++) {
        ...
        if (atleta.num == chave) {
            atleta.posicaoNaLista = i; // se achou, guarda nesse atributo a posição na lista (índice)
            return listaAtletas[i];//se achou, interrompe o laço de repetição e devolve a linha inteira
        }
    }
...
function inserir() {
    ...
    document.getElementById("inputAltura").value = 193;
    document.getElementById("inputPeso").value = 83.8;
}
...
function salvar() {
    ...
    let nascimento = parseFloat(document.getElementById("inputNota1").value);
    let posicao = parseFloat(document.getElementById("inputNota2").value);
    let altura = parseFloat(document.getElementById("inputNota3").value);
    let peso = parseFloat(document.getElementById("inputNota4").value);

    //verificar se o que foi digitado pelo USUÁRIO está correto
    if (!isNaN(parseFloat(num)) && num > 0 && num < 100 &&
        nome &&
...
function bloquearAtributos(valor) {
    ...
    document.getElementById("inputNum").readOnly = !valor; // sempre ao contrário dos outros atributos
...
function inserirDadosIniciais() {
    ...
    visibilidadeDosBotoes('inline', 'none', 'none', 'none', 'none');
    bloquearAtributos(true);
>>>>>>> 78cb197903966e6bee2fd38a81e1ebab1af66cf3
}